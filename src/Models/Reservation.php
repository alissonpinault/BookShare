...
class Reservation
{
    ...
    {
        return $this->data['utilisateur_id'] ?? null;
    }
    ...
    {
        return $this->data['date_reservation'] ?? null;
    }
    ...
    {
        $stmt = $this->pdo->prepare(
            'SELECT livre_id, statut FROM reservations WHERE reservation_id = ? AND utilisateur_id = ?'
        );
        ...
        try {
            ...
            $this->pdo->prepare('UPDATE reservations SET statut = ? WHERE reservation_id = ?')
                ->execute(['annulee', $reservationId]);

            $this->pdo->prepare('UPDATE livres SET disponibilite = ? WHERE livre_id = ?')
                ->execute(['disponible', $reservation['livre_id']]);
    ...
    private function getReservationsByStatut(
        int $utilisateurId,
        string $statut,
        ?int $limit,
        ?int $offset
